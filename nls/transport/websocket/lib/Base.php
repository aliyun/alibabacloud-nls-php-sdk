...
class Base implements LoggerAwareInterface
{
    ...
    public function setLock($name): void
    {
        $this->lock = new CacheLock($name);
    }

    public function unsetLock(): void
    {
        unset($this->lock);
    }
    ...
    public function receive($block = true)
    {
        ...
        do {
            $response = $this->receiveFragment($block);
            if ($head == true && $block == false && count($response) == 1)
            {
                // no data can read
    ...
    protected function receiveFragment($block = true): array
    {
        // Read the fragment "header" first, two bytes.
        $data = $this->read(2, $block);
        if ($block == false && strlen($data) < 1) {
            // no data can read
            //printf("receiveFragment no data\n");
            return [false];
        }
    ...
    public function close(int $status = 1000, string $message = 'ttfn'): void
    {
        ...
        $this->receive(false); // Receiving a close frame will close the socket now.
    }
    ...
    protected function read(string $length, bool $block = true): string
    {
        $data = '';
        $head = true;
        while (strlen($data) < $length) {
            // $socketInof = socket_get_status($this->socket);
            // printf("read can read %dbytes\n", $socketInof['unread_bytes']);
            // if ($head == true && $block == false) {
            //     if (strlen($data) < 1 && $socketInof['unread_bytes'] == 0) {
            //         printf("read no data\n");
            //         break;
            //     } else {
            //         $head = false;
            ...
            if ($buffer === '') {
                ...
                //printf("Empty read; connection dead?\n");
            }
            $data .= $buffer;
            $read = strlen($data);
