...
/**
 * Create the local service of RT-ASR
 * 创建实时语音识别请求的本地服务端, 用于与语音服务端进行通信
 */
class SpeechTranscriberWorker
{
    ...
    /**
     * SpeechTranscriberWorker 的构造函数
     *
     * @param string $port  本地服务端的websocket通信端口
     *
     * @return SpeechTranscriberWorker
     */
    public function __construct($port)
    {
        ...
        $this->_worker->name = 'SpeechTranscriberWorker';
        ...
        $this->_worker->onMessage = function($connection, $data)
        {
            ...
            else if ($this->_curStatus === 'Working')
            {
                if (isset($this->_webClient))
                {
                    ...
                    else if (is_string($data) && $data == 'StopTranscription')
                    {
                        // 需要断开链接
            ...
            else if ($this->_curStatus === 'Stopping')
            {
                if (isset($this->_webClient))
                {
                    ...
                    else if ($data === 'StoppedTranscription')
                    {
                        $this->_curStatus = 'Idle';
                    }
                    else
                    {
                        ...
                        {
                            // receive zero data
                            Console::debug(sprintf("[%s] stopping, cannot get any response", $this->_worker->name));
                        }
                        if (is_string($response))
                        {
                            Console::debug(sprintf("[%s] stopping, receive response: $response", $this->_worker->name));
                            $connection->send($response);
                        }
                    }
                }
